...
class ERANDB(object):
    ...
    def getData(self):
        """returns the values the robots are broadcasting

        Returns:
            list: list of the values each robot is broadcasting
        """
        readings = self.robot.epuck_range_and_bearing.get_readings()
        return [reading[0] for reading in readings]

    def getRanges(self):
        """gets the list of distance to each robot.

        Returns:
            list: distance to each robot
        """
        readings = self.robot.epuck_range_and_bearing.get_readings()
        return [reading[1] for reading in readings]

    def getBearings(self):
        """gets the list of bearing (angles) with each robot

        Returns:
            list: angles with each robot
        """
        readings = self.robot.epuck_range_and_bearing.get_readings()
