...
class Logger(object):
    ...
    """
    def __init__(self, logfile, header, rate = 0, buffering = 3, ID = None):
        self.logfile = logfile
        self.header = header
        self.buffering = buffering
        self.file = open(logfile, 'w+', buffering = buffering*len(header))
        # self.rate = rate
        # self.tStamp = 0
        # self.tStart = 0
        # self.latest = time.time()
        pHeader = ','.join([str(x) for x in header])
        self.file.write(f'{pHeader}\n')
        self.close()
    ...
    def begin(self):
        self.file = open(self.logfile, 'a', buffering = self.buffering*len(self.header))
